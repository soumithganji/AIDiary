...
async def startup_event():
    """Initialize database and all components on startup"""
    print("\n" + "=" * 60)
    print("AIDiary - Your Private Creative Companion")
    print("=" * 60)
    print(f"\n  Access at: http://localhost:8000")
    print("=" * 60 + "\n")
    
    # Auto-initialize database (no password required)
    try:
        db = DiaryDatabase()
        ...
        # Load AI model (this might take a moment)
        try:
...
# Global state
app_state = {
    "unlocked": True,  # Always unlocked (no password)
    "db": None,
    "rag": None,
...
async def get_status():
    ...
    return {
        "ready": app_state.get("db") is not None,
...
async def chat(request: ChatSessionRequest):
    ...
    if not qwen:
        ...
        return {
            "response": "The AI model is not currently loaded. This could be because:\n\n"
                       ...
                       "You can still use AIDiary for journaling, mood tracking, and pattern analysis. "
    ...
    try:
        ...
        previous_messages = db.get_chat_messages(session_id, limit=10)
        # Messages come newest first, reverse and exclude the current user message we just added
        for msg in reversed(previous_messages[1:]):  # Skip the message we just added
...
async def get_daily_greeting():
    ...
    try:
        ...
        # Use fast time-based greeting from recommender (no AI generation for speed)
        greeting = suggestions_data.get("greeting", "Hello!")
...
async def get_optional_auth(authorization: Optional[str] = Header(None)) -> bool:
    """Check if user is authenticated via desktop unlock"""
    return app_state["unlocked"]
...
if frontend_dir.exists():
    ...
    async def serve_frontend():
        ...
        return {"message": "AIDiary API is running"}
